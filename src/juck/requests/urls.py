...
urlpatterns = patterns('juck.requests.views',
    ...
    url(r'^conversation/(?P<req_id>\w+)/(?P<resp_id>\d+)/$', 'request_conversation', {}, name='request_conversation'),
    ...
    url(r'^add/(?P<request_type>\w+)/(?P<item_pk>\d+)/$', 'add_request', {}, name='add_request'),

    url(r'^req_status/(?P<request_type>\w+)/(?P<item_pk>\d+)/$', 'view_request_status', {}, name='request_status'),
)
